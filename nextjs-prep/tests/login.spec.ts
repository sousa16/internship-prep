...
test('empty username', async ({ page }) => {
  ...
  const isUsernameFocused = await page.evaluate(() => document.activeElement.id === 'username');
...
test('empty password', async ({ page }) => {
  ...
  // Check if the second input (password) is focused
  // This indicates that the browser's validation is prompting the user to fill out this field
  const isPasswordFocused = await page.evaluate(() => document.activeElement.id === 'password');
